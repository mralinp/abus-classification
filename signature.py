...
def signature(binary_image, res=2):
    binary_image = utils.image.rotation_invariant(binary_image)
    x0, y0 = utils.image.find_shape_center(binary_image)
    c = x0 - y0
    boundary = utils.image.get_boundary(binary_image)
